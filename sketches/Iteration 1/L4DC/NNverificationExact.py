...
for trial in trials:
    ...
    lossActual  = sum(i >= 0 for i in output_data) / numSamples
    ...
    if trial != 0:
        ...
        print('Probability of Success Actual: ', "%.2f" % round(lossActual[0] * 100, 3), '%')
        ...
        if loss >= gamma and (1 - lossActual[0]) >= gamma:
...
plotECFandCDF(grid, CDFgrid_output, CFout, CDFout, axs[0:3, 2], 2)
plotEmpiricalCDF(output_data, axs[2, 2], 2)
