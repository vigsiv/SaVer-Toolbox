...
class dkw:
    ...
    def findZeroOne(self):
        ...
        self.zeroOne[0] = optimize.root_scalar(self.empiricalCDF, method = 'brentq', bracket = [-100,100], xtol=self.tol).root
        complementZero = lambda x: 1-self.empiricalCDF(x)
        self.zeroOne[1] = optimize.root_scalar(complementZero,method = 'brentq', bracket = [-100,100], xtol=self.tol).root
